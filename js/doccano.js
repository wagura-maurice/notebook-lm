...
class DoccanoApp {
  constructor() {
    ...
    this.structureHandlers = {
      ...
      heading: (content, doc) => {
        ...
        return `<${headingTag} class="${className}">${this.stripMarkdown(content)}</${headingTag}>`;
      },
  ...
  displayAllDocuments() {
    ...
    const mainTitleDoc = this.documentData.find(d => 
      d.header && d.header.includes('AI ADOPTION IN THE PUBLIC SECTOR')
    );
    ...
    if (canvasTitle) {
      canvasTitle.textContent = mainTitleDoc ? 
        this.stripMarkdown(mainTitleDoc.header) : 'AI ADOPTION IN THE PUBLIC SECTOR';
    }
    ...
    this.documentData.forEach((doc, index) => {
      ...
      if (doc.header) {
        ...
        if (currentHeader !== lastHeader) {
          ...
          const headerContent = this.structureHandlers.heading.call(this, doc.header, { header: doc.header });
  ...
  formatText(text, taxonomy = {}) {
    ...
    formatted = formatted
      .replace(/^#\s+(.*$)/gm, '<h2 class="text-xl font-semibold text-eu-blue mt-6 mb-3">$1</h2>')
      .replace(/^##\s+(.*$)/gm, '<h3 class="text-lg font-semibold text-eu-blue mt-5 mb-2">$1</h3>')
      .replace(/^###\s+(.*$)/gm, '<h4 class="font-semibold text-eu-blue mt-4 mb-2">$1</h4>');
  ...
  initializeDocumentInteractions() {
    ...
    const taxonomyHighlights = this.documentContainer?.querySelectorAll('.taxonomy-highlight');
    ...
    const documentElements = this.documentContainer?.querySelectorAll('.document-element');
    if (documentElements) {
