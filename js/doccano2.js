...
class DoccanoApp {
  ...
  countTaxonomyItems() {
    ...
    this.documentData.forEach((doc, index) => {
      ...
      console.log(
        `Document ${index} taxonomy structure:`,
      ...
      Object.keys(initialCounts).forEach((type) => {
        ...
        if (Array.isArray(taxonomy[type])) {
          ...
          console.log(
            `Found ${count} items (total: ${this.taxonomyCounts[type]})`
          );
          if (count > 0) {
          ...
          this.taxonomyCounts[type] += count;
          console.log(
            `Found ${count} items in object format (total: ${this.taxonomyCounts[type]})`
          );
          if (count > 0) {
            console.log(
              "Sample keys:",
              Object.keys(taxonomy[type]).slice(0, 2)
            );
          }
        } else {
          console.warn(
            `Unexpected data type for ${type}:`,
            typeof taxonomy[type]
          );
        }
    ...
    Object.entries(this.taxonomyCounts).forEach(([taxonomyType, count]) => {
      ...
      if (!countSpan) {
        ...
        if (!countSpan) {
          ...
          countSpan.className =
            "taxonomy-count bg-eu-blue/10 text-eu-blue text-xs font-medium px-2 py-0.5 rounded-full ml-2";
      ...
      if (countSpan) {
        ...
        console.error(
          `❌ Failed to find or create count span for: ${taxonomyType}`
        );
  ...
  processDocumentElement(doc, index) {
    ...
    wrapper.innerHTML = handler.call(
      this,
      content,
      doc,
  ...
  assignTaxonomy(_, taxonomyType) {
    ...
    const selectionId = `sel-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
    ...
    while (
      lineElement &&
      lineElement !== docElement &&
      !["P", "H1", "H2", "H3", "H4", "H5", "H6", "DIV", "LI"].includes(
        lineElement.nodeName
      )
    ) {
    ...
    const selectionData = {
      ...
      context: {
        elementType: lineElement
          ? lineElement.nodeName.toLowerCase()
          : "unknown",
        elementText: lineElement
          ? lineElement.textContent.substring(0, 100) +
            (lineElement.textContent.length > 100 ? "..." : "")
          : "",
    ...
    highlightSpan.title = `${this.getTaxonomyDisplayName(
      taxonomyType
  ...
  applySingleHighlight(span, selectionData, taxonomyType) {
    ...
    span.addEventListener("dblclick", async (e) => {
      const result = await Swal.fire({
        ...
        customClass: {
          confirmButton:
            "bg-eu-blue hover:bg-eu-blue-dark text-white font-medium py-2 px-4 rounded-md",
          cancelButton:
  ...
  async undoLastHighlight() {
    ...
    const highlightElement = document.querySelector(
      `[data-selection-id="${lastHighlight.selectionId}"]`
    );
    ...
    if (highlightElement) {
      ...
      lastHighlight.path =
        lastHighlight.path || this.getNodePath(highlightElement);
  ...
  async redoLastUndo() {
    ...
    if (!exists) {
      ...
      if (targetNode && targetNode.nodeType === Node.TEXT_NODE) {
        ...
        if (start >= 0 && end <= text.length && actualText === expectedText) {
          ...
          if (parent) {
            ...
            this.applySingleHighlight(
              span,
              selectionData,
              lastUndone.taxonomyType
            );
      ...
      if (!highlightApplied) {
        console.warn(
          "Could not find exact position, falling back to text search"
        );
      ...
      if (!highlightApplied) {
        ...
        const index = doc.enrichment.taxonomy[
          lastUndone.taxonomyType
        ].findIndex((item) => item.id === lastUndone.selectionId);
        if (index !== -1) {
  ...
  saveHighlights() {
    ...
    this.documentData.forEach((doc, docIndex) => {
      if (doc.enrichment?.taxonomy) {
        Object.entries(doc.enrichment.taxonomy).forEach(
          ([taxonomyType, items]) => {
            items.forEach((item) => {
              highlights.push({
    ...
    Toast.fire({
      icon: "success",
      title: `Saved ${highlights.length} highlight${
